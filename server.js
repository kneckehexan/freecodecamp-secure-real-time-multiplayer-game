...
let foodEntity = new Collectible({x: foodX, y: foodY, value: 1, id: uuidv4()});
let [mineX, mineY] = getRandomPos();
let mineEntity = new Player({x: mineX, y: mineY, score: 1, id: uuidv4()});
let connections = [];
...
io.sockets.on('connection', socket => {
  ...
  let player = new Player({x: posX, y: posY, score: 0, id: socket.id});
  players.push(player);

  socket.emit('init', {id: socket.id, players: players, foodItem: foodEntity, mineItem: mineEntity});
  ...
  socket.on('update', (updateUser) => {
    ...
    io.emit('update', {players: players, mine: mineEntity, food: foodEntity, player: null});
  });
